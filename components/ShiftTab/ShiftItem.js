...
const ShiftItem = (item,navigation) => {
    ...
    useEffect(()=>{
        console.log("current item is",item.item);
    },[])
        ...
        <Swipeable>
            <TouchableOpacity style={{marginHorizontal:2, marginTop:5}} onPress={()=>setExpanded(!expanded)}>
                <View style={{flexDirection:"column"}}>
                    ...
                    <View style={{flexDirection:"row"}}>
                        <View style={{flex:1}}>
                            <Button color={"#8c8c8c"} title="edit" onPress={()=>editShift(item.item.index)}/>
                        </View>
                        <View style={{flex:1}}>
                            <Button color={"#6e0006"} title="delete" onPress={()=>deleteShift(item.item.index)}/>
                        </View>
                    </View>
                    </>
                    }
