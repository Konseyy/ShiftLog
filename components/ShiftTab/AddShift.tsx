...
const AddShift: FC<any> = ({navigation, route}) => {
  ...
  function onSelect(event: Event, selectedDate: Date | undefined, dateSetter:(date:Date)=>void, modalSetter:React.Dispatch<React.SetStateAction<boolean>>){
    modalSetter(false);
    if(!selectedDate){
      return;
    }
  ...
  async function addShift(){
    ...
    if(!dateCheck(dateToAddStart.getTime(),dateToAddEnd.getTime())){
	 ...
	 if(!params.current){
		await modifyShifts({type:"add",value:{data:{
		...
		await modifyShifts({type:"edit",value:{data:{
			startTime:dateToAddStart.getTime(),
			endTime: dateToAddEnd.getTime(),
			break: breakTime,
			notes: notes,
  ...
  return (
    <>
      ...
      {showStartDateSelect&&<DateTimePicker mode="date" value={startDate} onChange={(e:Event,s:Date | undefined)=>onSelect(e,s,setStartDate,setShowStartDateSelect)}/>}
      {showStartTimeSelect&&<DateTimePicker mode="time" value={startTime} onChange={(e:Event,s:Date | undefined)=>onSelect(e,s,setStartTime,setShowStartTimeSelect)}/>}
      {showEndDateSelect&&<DateTimePicker mode="date" value={endDate} onChange={(e:Event,s:Date | undefined)=>onSelect(e,s,setEndDate,setShowEndDateSelect)}/>}
      {showEndTimeSelect&&<DateTimePicker mode="time" value={endTime} onChange={(e:Event,s:Date | undefined)=>onSelect(e,s,setEndDateTime,setShowEndTimeSelect)}/>}
    </>
